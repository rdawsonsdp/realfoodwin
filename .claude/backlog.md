...
## Personalization & measurement — deferred

- **Personalization engine** (added 2026-05-22) — Re-rank library matcher's top-K candidates using precomputed signals so the swap path stays sub-2s. Needs: `users.taste_embedding vector(1024)` (offline-updated weighted mean of loved swaps minus dismissed); `user_neighbors` cache (daily-refreshed nearest neighbors by taste); inline re-rank function combining (personal saves boost, cohort lift, dismissal penalty, taste cosine). Cold-start fallback to popularity for <3 made-it events.
- **Factorial A/B framework for personalization factors** (added 2026-05-22) — Test which *factors* in the recommender move the needle, not just personalized-vs-not. Needs: `experiment_assignments` table `(user_id, experiment_name, variant, assigned_at)` with deterministic `hash(user_id || experiment_name)` assignment; `swaps.experiment_factors jsonb` logging which factors were active per swap and each factor's actual rank contribution; admin pivot at `/admin/experiments` showing save-rate / loved-rate / made-it-rate per factor (on vs off) with sample size + p-value, plus a 2D heatmap of pairwise interactions. Start with 3–4 binary factors (personal_history, cohort, taste_vector, dismissal_penalty) — at 5000 users that's ~300/cell, enough to detect ~5pp lift at 80% power. Pairwise interactions need ~20k users for confident detection.
- **Per-arm satisfaction split** (added 2026-05-22) — Once the personalization engine ships, add an arm-split to `/admin/satisfaction` so the composite score (loved-ratio 40% / avg-rating 30% / made-it rate 20% / save-rate 10%) can be read separately for treatment vs control. Add a weekly trend chart of the same composite so lift after a deploy is visible.
- **Shadow-ranking logging** (added 2026-05-22) — Even when the control ranking is served, compute and store the personalized ranking too. Then evaluate offline with NDCG@5 / MRR against the items the user actually engaged with. Gives signal in days, not the weeks the A/B needs, and catches regressions before they ship to everyone.
